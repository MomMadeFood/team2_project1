	...
	function searchBtn(){
		...
		if(startDate && !endDate){
			...
			$.ajax({
				...
				htmlContent += '    <col width="15%"/>';
				htmlContent += '	<col width="42%"/>';
				htmlContent += '	<col width="7%"/>';
				htmlContent += '	<col width="12%"/>';
				htmlContent += '	<col width="12%"/>';
				htmlContent += '	<col width="12%"/>';
				htmlContent += '  </colgroup>';
					...
					htmlContent += '  	<tbody class="txt">';
						for(var i = 0 ; i < order['detailList'].length ; i++){
							...
							if(i==0){
								...
								htmlContent += '  			  <a style="color: #ecd795" href="${pageContext.request.contextPath}/order/orderDetail?orderNo="'+ order.orderNo +'">상세보기</a>';
								htmlContent += '  		    </td>';
							}
							...
							var orderState = order['detailList'][i].state;
							var orderStateStr = "";
							var orderCancelBtn ="";
							if(orderState==1){
								orderStateStr = "입금대기중";
								orderCancelBtn ='<button class="btn btn-sm btn-outline-secondary btn-search">주문취소</button>	';
							...
							htmlContent += '  		   <td style="border-left: 1px solid #E5E5E5; vertical-align: middle; text-align:center"><p>'+orderStateStr+'</p></td>';
							htmlContent += '  		   <td style="border-left: 1px solid #E5E5E5; vertical-align: middle; text-align:center"><p>'+orderCancelBtn+'</p></td>';
							htmlContent += '  		 </tr>';
						}
					htmlContent += '  	 </tbody>';
				}
